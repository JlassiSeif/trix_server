...
End-to-end: run a built server with fast bots (`TRIX_SPEED=10 PORT=8123 npm start`), then `npm run e2e -- --base http://127.0.0.1:8123`. It plays a whole game in Chromium through the UI and saves screenshots to `e2e-out/`. If Playwright's own Chromium isn't installed, point `CHROMIUM=` at a Chrome/Chromium binary.
