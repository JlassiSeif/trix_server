...
- **Every game has an icon, one deck in the kit, one plan (2026-09-25):** an icon per game in the house style (in each game's description, on the home page's cards, the game page and the lobby); the full deck in the kit for every card game, in the 2023 style: Seif's 2023 cards come from Aisleriot's original "bonded" deck (Ubuntu replaces that file with a different design, which was tried the same day and Seif hated it, so it was undone); 7 to ace are the 2023 files, 2 to 6 and the jokers were cut from the original the same way (`platform/ui/tools/cards-from-aisleriot.mjs`); `docs/games.md` as the one page per game.
