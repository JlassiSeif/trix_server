...
5. **Card faces 2 to 6 and jokers, in the 2023 style** (needed by Chkobba, Rami, Pablo, Tekdheb): Seif, where did the 2023 cards come from? With the source we render the missing cards in the same style; Seif rejected the "bonded" deck's look (2026-09-25).
6. **Own machine before going public** (architecture §11). Which machine? Seif's Oracle free account?
7. **Dineri's own card faces** (Seif, 2026-09-25: later). Trix uses GNOME Aisleriot's faces (GPL, credited on the About page and in the README).
...
- **Every game has an icon, one deck in the kit, one plan (2026-09-25):** an icon per game in the house style (in each game's description, on the home page's cards, the game page and the lobby); the card faces moved into the kit for every card game to share (the 2023 faces: a sharper re-render from a different Aisleriot deck was tried the same day and Seif hated it, so it was undone); `docs/games.md` as the one page per game.
...
- **Last full check (2026-09-25, icons and the shared deck; not deployed):** unit tests 153/153, typecheck and build clean, browser flows 17/17, full games against bots at desktop and phone size with the new deck, the hub pages and every icon checked in screenshots. The live release `fe275c1` was checked in full before its deploy (see Deployed).
