...
- [x] **Testing station + bug hunt** (2026-09-23): `npm run station` (19 scenarios, independent referee, 4-player cross-checks, 25-table stress) and `tools/station/src/mutants.ts` (caught 7 of 7 deliberate bugs). It found 2 server bugs, both fixed with regression tests: a refused createRoom/joinRoom pulled a seated player out of their seat, and a message flood froze the server (now rate-limited). Structured JSON logs, request ids in errors, `/api/stats`. Full write-up: `docs/testing-station.md`.
- [x] **Pre-deployment check** (2026-09-23): `docs/predeploy-check.md`. Fixed: games lost on restart (rooms now saved to `TRIX_STATE_FILE` and restored), a frozen page when a table is gone, two tabs fighting over one seat, leaving without confirmation. Added caps (200 rooms, 1000 sockets), security and cache headers, and a self-contained server build. Verified: 20/20 station scenarios, 100 tables at once, 11/11 browser connection flows. Verdict: ready from the software side.
- [x] **Ownership** (2026-09-23), R-TABLE-12: ownership passes on after the owner leaves or is away 30 s, and the owner can hand it over. Tested by 7 server tests and browser C11/C12.
- [x] **Security review** (2026-09-23): `docs/security.md`. Per-address limits, a lockout for guessed links and tokens, an origin check, no duplicate names, HTTP hardening. Station X01–X10 pass; npm audit clean.
- [x] **Phones** (2026-09-23): Seif confirmed the game works on his real phone over the home network. Decision: friends will play on PCs, so no deeper phone verification.
- [ ] **Deploy** (M5): waiting for Seif's instructions. Drafts: `docs/deploy.md`, `deploy/trix.service`, `deploy/Caddyfile`.
