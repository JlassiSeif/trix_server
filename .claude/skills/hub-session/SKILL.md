...
- **Every game follows the house style** (`docs/game-look.md`, Seif 2026-09-25: "one house, many rooms"). Read it before building or changing any game's screens, and before starting a new game. The frame, type, colours, controls, panels, seats and card backs come from the kit and the brand; a game only owns its playing surface, pieces, one accent from the brand's colours, its icons and later its sounds. No game gets its own logo, fonts, button styles or colours. Changing the guideline itself needs Seif's OK and a dated line in it.
...
| Playing cards: drag and drop, premoves (Trix) | `PORT=8126 node platform/server/dist/index.js` (normal speed, so there's time to premove) then `node platform/web/e2e/premove.mjs OUT --viewport 390x844` (and 360x740, and desktop) |
| A game's screens, or a new game | Follow `docs/game-look.md`; `npx vitest run --root platform/ui` (the look test: brand colours and fonts only); then look at screenshots: full games at 1280×800, 390×844 and 360×740, and the table in Arabic |
