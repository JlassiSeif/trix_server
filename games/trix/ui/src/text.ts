...
export const T = texts({
  en: {
    ...
    premove: {
      title: (card: string) => `Premove ${card}: it plays on your turn`,
      cancelTitle: "Your premove: tap to cancel",
      queued: (card: string) => ` · premove ${card}`,
      cancelled: (card: string) => `Premove cancelled: ${card} can't be played`,
    },
    scores: "Scores",
  ...
  fr: {
    ...
    premove: {
      title: (card: string) => `Jouer ${card} d'avance : la carte part à votre tour`,
      cancelTitle: "Votre coup d'avance : touchez pour l'annuler",
      queued: (card: string) => ` · d'avance : ${card}`,
      cancelled: (card: string) => `Coup d'avance annulé : ${card} ne peut pas être joué`,
    },
    scores: "Scores",
  ...
  ar: {
    ...
    premove: {
      title: (card: string) => `العب ${ltr(card)} مسبقًا: تُلعب في دورك`,
      cancelTitle: "لعبك المسبق: انقر للإلغاء",
      queued: (card: string) => ` · لعب مسبق: ${ltr(card)}`,
      cancelled: (card: string) => `أُلغي اللعب المسبق: لا يمكن لعب ${ltr(card)}`,
    },
    scores: "النقاط",
    declareKing: "أعلن K♥",
