...
//
//   npx tsx games/trix/station/src/mutants.ts
...
interface Mutant {
  name: string;
  file: string; // relative to games/trix/engine/src
  from: string;
  to: string;
...
for (const [i, m] of MUTANTS.entries()) {
  ...
  cpSync(join(root, "games/trix/engine/src"), join(dir, "engine"), { recursive: true });
  ...
  execFileSync(esbuild, [
    join(root, "platform/server/src/index.ts"), "--bundle", "--platform=node", "--format=esm", "--target=node22", "--external:ws",
    `--alias:@games/trix=${join(dir, "engine/index.ts")}`, `--outfile=${join(dir, "server.js")}`, "--log-level=warning",
  ]);
  const run = spawnSync(process.execPath, ["--import", "tsx", join(root, "games/trix/station/src/main.ts"), "--only", "S01,S02,S21", "--seed", String(100 + i), "--server-js", join(dir, "server.js"), "--out", join(dir, "run")], {
    cwd: root,
    encoding: "utf8",
    timeout: 300_000,
  });
