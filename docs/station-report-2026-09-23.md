...
- Run: 2026-09-23T13-27-15 · seed 21 · speed ×20 · 25 tables in the stress case · 201 s
- Server: http://127.0.0.1:37735 · logs: `server.log` and `clients/*.jsonl` in this folder
- **19/19 scenarios passed**

| | ID | Scenario | Got |
|---|---|---|---|
| ✅ | S01 | One full game, four players | game over after 2 contracts (overLimit), totals 20/50/1050/220 |
| ✅ | S02 | Many tables at once | 25/25 tables finished in 14.8 s. Rooms 1 → 26 → 1; sockets 100 → 0; heap 10.6 → 12.6 → 18.3 MB |
| ✅ | S03 | Play again after the game | first: game over after 16 contracts (overLimit), totals 540/980/1010/960; second: game over after 5 contracts (overLimit), totals 1020/210/350/370 |
| ✅ | S04 | Two players and two server bots | game over after 20 contracts (overLimit), totals 380/830/1110/720 |
| ✅ | S05 | Nobody clicks Continue | gaps between contracts: 501 ms, 499 ms, 499 ms (target 500 ms); game over after 13 contracts (overLimit), totals 610/1050/210/830 |
| ✅ | S10 | Players spam moves out of turn | 923 moves sent out of turn: 921 refused, 2 accepted because they had become legal on arrival (each checked by the referee); game over after 22 contracts (overLimit), totals 780/700/930/1180 |
| ✅ | S11 | Junk and malformed messages | game over after 28 contracts (allContractsPlayed), totals 590/830/970/610 |
| ✅ | S12 | Message over the 4 KB limit | connection closed: true (code 1009); table paused: true; resumed after reconnect: true; game over after 13 contracts (overLimit), totals 660/370/510/1140 |
| ✅ | S13 | Every move sent twice (double click) | 347 moves sent twice, 347 second copies refused; game over after 11 contracts (overLimit), totals 1280/630/380/760 |
| ✅ | S22 | Message flood while a table plays | outsider cut off: true after 81 replies; flooding player cut off: true; flood took 224 ms; health p95 7 ms, max 7 ms over 15 probes; game over after 20 contracts (overLimit), totals 1390/730/130/570 |
| ✅ | S14 | A player keeps dropping and coming back | pauses 15, resumes 15, same hand 15; game over after 20 contracts (overLimit), totals 1090/550/940/500 |
| ✅ | S15 | Owner plays on with a bot, then the player returns | bot played ≥3 cards for the missing player; player back in control: true; game over after 24 contracts (overLimit), totals 590/1530/850/580 |
| ✅ | S16 | Kick in the middle of a game, replacement joins | kicked player removed; table paused: true; old token/old link: BAD_TOKEN, BAD_INVITE; new link differs: true; replacement took seat 3 with 0 cards; game over after 15 contracts (overLimit), totals 1030/340/570/70 |
| ✅ | S17 | The owner leaves in the middle of a game | new owner: seat 1; has invite link: true; paused: true; game over after 20 contracts (overLimit), totals 1010/870/610/510 |
| ✅ | S18 | The same seat opened in a second tab | first tab told "REPLACED": true; first tab disconnected: true; second tab seat 2; game over after 28 contracts (overLimit), totals 850/1200/560/200 |
| ✅ | S19 | Everyone drops at once, then everyone returns | rooms 15 → 15 while away; game unchanged on return: true; game over after 22 contracts (overLimit), totals 1300/820/380/620 |
| ✅ | S20 | Refused joins and owner-only actions | 11/11 refused with the expected code |
| ✅ | S21 | Looking at the last trick: limit and privacy | looks shown 2, refusals CANNOT_PEEK (still same contract: true); game over after 4 contracts (overLimit), totals 350/140/180/1180 |
| ✅ | S23 | Lobby: end a game, kick, add a bot, start again | back to lobby: true; new game started on its own: true; game over after 20 contracts (overLimit), totals 140/70/480/1040 |
...
**Got:** game over after 2 contracts (overLimit), totals 20/50/1050/220
...
**Got:** 25/25 tables finished in 14.8 s. Rooms 1 → 26 → 1; sockets 100 → 0; heap 10.6 → 12.6 → 18.3 MB
...
**Got:** first: game over after 16 contracts (overLimit), totals 540/980/1010/960; second: game over after 5 contracts (overLimit), totals 1020/210/350/370
...
**Got:** game over after 20 contracts (overLimit), totals 380/830/1110/720
...
**Got:** gaps between contracts: 501 ms, 499 ms, 499 ms (target 500 ms); game over after 13 contracts (overLimit), totals 610/1050/210/830
...
**Got:** 923 moves sent out of turn: 921 refused, 2 accepted because they had become legal on arrival (each checked by the referee); game over after 22 contracts (overLimit), totals 780/700/930/1180

Checks:
- ✅ nearly all refused (921/923)
- ✅ all players see the same final result (1 distinct)
...
**Got:** connection closed: true (code 1009); table paused: true; resumed after reconnect: true; game over after 13 contracts (overLimit), totals 660/370/510/1140
...
**Got:** 347 moves sent twice, 347 second copies refused; game over after 11 contracts (overLimit), totals 1280/630/380/760

Checks:
- ✅ every second copy refused (347/347)
- ✅ all players see the same final result (1 distinct)
...
**Got:** outsider cut off: true after 81 replies; flooding player cut off: true; flood took 224 ms; health p95 7 ms, max 7 ms over 15 probes; game over after 20 contracts (overLimit), totals 1390/730/130/570

Checks:
- ✅ outsider cut off by the rate limit
- ✅ flooding player cut off by the rate limit
- ✅ health p95 under 100 ms (got 7)
- ✅ all players see the same final result (1 distinct)
...
**Got:** pauses 15, resumes 15, same hand 15; game over after 20 contracts (overLimit), totals 1090/550/940/500

Checks:
- ✅ every drop paused and every return resumed (15/15)
...
**Got:** bot played ≥3 cards for the missing player; player back in control: true; game over after 24 contracts (overLimit), totals 590/1530/850/580
...
**Got:** kicked player removed; table paused: true; old token/old link: BAD_TOKEN, BAD_INVITE; new link differs: true; replacement took seat 3 with 0 cards; game over after 15 contracts (overLimit), totals 1030/340/570/70
...
**Got:** new owner: seat 1; has invite link: true; paused: true; game over after 20 contracts (overLimit), totals 1010/870/610/510
...
**Got:** first tab told "REPLACED": true; first tab disconnected: true; second tab seat 2; game over after 28 contracts (overLimit), totals 850/1200/560/200
...
**Got:** rooms 15 → 15 while away; game unchanged on return: true; game over after 22 contracts (overLimit), totals 1300/820/380/620
...
**Got:** looks shown 2, refusals CANNOT_PEEK (still same contract: true); game over after 4 contracts (overLimit), totals 350/140/180/1180
...
**Got:** back to lobby: true; new game started on its own: true; game over after 20 contracts (overLimit), totals 140/70/480/1040
...
26872 lines: info 2763, debug 24019, warn 90

| level + event | count |
|---|---|
| debug action.accepted | 23026 |
| info action.rejected | 1352 |
| info contract.scored | 710 |
| debug bot.move | 590 |
| debug ws.open | 203 |
| debug ws.close | 200 |
| info seat.joined | 172 |
| info room.status | 132 |
| info seat.vacated | 103 |
| info seat.disconnected | 88 |
| warn message.bad | 87 |
| info game.started | 45 |
| info room.created | 44 |
| info game.over | 43 |
| info room.closed | 25 |
| info seat.reconnected | 23 |
| info request.refused | 20 |
| info seat.botAdded | 3 |
| warn ws.rateLimited | 2 |
| info seat.botStandIn | 2 |
| info server.listening | 1 |
| warn ws.error | 1 |
